...
## Collecciones
Desde terminal primero ejecutar ```mongosh```
```mongosh
...
- Filtrar por campo
```mongosh
db.alumnos.find({ alumnoRegular: true })
db.alumnos.find({ edad: { $gte: 20, $lte: 23 } })
```

- Buscar por campo
```mongosh
db.profesores.findOne({ nombre: "Andrea Torres" })
db.clases.find({ encargado: "dd9c691a-d9b5-420e-bb43-4e83fb8aa8d0" })
```
